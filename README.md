...
- 每日新字按儿童启蒙顺序优先，不直接沿用规范字表原始顺序，前期会先出现人、口、手、日、月、山、水等容易感知和造句的字。
- 每个汉字都会记录出现次数、答对次数、答错次数、最近学习时间和掌握状态，用于后续复习和难度调整。
- 自动生成字条目会使用常用词语和儿童化短句，不再使用“X字 / 认识X / 写X”这类占位组词；极少数不适合儿童组词的字会先用单字兜底。
- 未绑定手机号时，记录按设备保存。
...
- 分数、星星、金币、连击
- 关卡星级记录与解锁判断
- 30 个装扮奖励，装扮间分页展示，最高装扮价格不超过 120 一一金币
...
系统会从 `src/game/data/commonChars.ts` 自动生成完整常用字题库，并从 `src/game/data/charUsage.ts` 读取组词、释义和例句。若想给某个字补充更适合宝一一的拼音、解释、词语和例句，可以在 `src/game/data/chars.ts` 的 `curatedCharItems` 数组中追加一项：
